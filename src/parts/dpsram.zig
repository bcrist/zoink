        ...
        pub fn pin(self: @This(), pin_id: Pin_ID) Net_ID {
            return switch (Pkg) {
                packages.TQFP_100 => if (addr_bits >= 15 and byte_bits == 8) switch (@intFromEnum(pin_id)) {
                    ...
                    7 => switch (addr_bits) {
                        15 => .no_connect,
                        16 => self.left.addr[15],
                        else => unreachable,
                    },

                    84 => self.right.addr[0],
                    83 => self.right.addr[1],
                    82 => self.right.addr[2],
                    81 => self.right.addr[3],
                    80 => self.right.addr[4],
                    79 => self.right.addr[5],
                    78 => self.right.addr[6],
                    77 => self.right.addr[7],
                    76 => self.right.addr[8],
                    75 => self.right.addr[9],
                    74 => self.right.addr[10],
                    73 => self.right.addr[11],
                    72 => self.right.addr[12],
                    71 => self.right.addr[13],
                    70 => self.right.addr[14],
                    69 => switch (addr_bits) {
                        15 => .no_connect,
                        16 => self.right.addr[12],
                        else => unreachable,
                    },

                    37 => self.left.lower_data[0],
                    36 => self.left.lower_data[1],
                    34 => self.left.lower_data[2],
                    33 => self.left.lower_data[3],
                    32 => self.left.lower_data[4],
                    31 => self.left.lower_data[5],
                    30 => self.left.lower_data[6],
                    29 => self.left.lower_data[7],

                    27 => self.left.upper_data[0],
                    26 => self.left.upper_data[1],
                    25 => self.left.upper_data[2],
                    24 => self.left.upper_data[3],
                    23 => self.left.upper_data[4],
                    22 => self.left.upper_data[5],
                    21 => self.left.upper_data[6],
                    20 => self.left.upper_data[7],

                    39 => self.right.lower_data[0],
                    40 => self.right.lower_data[1],
                    41 => self.right.lower_data[2],
                    42 => self.right.lower_data[3],
                    43 => self.right.lower_data[4],
                    44 => self.right.lower_data[5],
                    45 => self.right.lower_data[6],
                    47 => self.right.lower_data[7],

                    48 => self.right.upper_data[0],
                    49 => self.right.upper_data[1],
                    51 => self.right.upper_data[2],
                    52 => self.right.upper_data[3],
                    53 => self.right.upper_data[4],
                    54 => self.right.upper_data[5],
                    55 => self.right.upper_data[6],
                    56 => self.right.upper_data[7],
                    ...
                    83 => self.right.addr[0],
                    82 => self.right.addr[1],
                    81 => self.right.addr[2],
                    80 => self.right.addr[3],
                    79 => self.right.addr[4],
                    78 => self.right.addr[5],
                    77 => self.right.addr[6],
                    76 => self.right.addr[7],
                    75 => self.right.addr[8],
                    74 => self.right.addr[9],
                    73 => self.right.addr[10],
                    72 => self.right.addr[11],
                    71 => self.right.addr[12],
                    70 => self.right.addr[13],
                    69 => self.right.addr[14],
                    68 => switch (addr_bits) {
                        15 => .no_connect,
                        16 => self.right.addr[12],
                        else => unreachable,
                    ...
                    30 => self.left.lower_data[6],
                    29 => self.left.lower_data[7],
                    27 => self.left.lower_data[8],

                    26 => self.left.upper_data[0],
                    25 => self.left.upper_data[1],
                    24 => self.left.upper_data[2],
                    23 => self.left.upper_data[3],
                    22 => self.left.upper_data[4],
                    21 => self.left.upper_data[5],
                    20 => self.left.upper_data[6],
                    18 => self.left.upper_data[7],
                    17 => self.left.upper_data[8],
                    ...
                    45 => self.right.lower_data[6],
                    47 => self.right.lower_data[7],
                    48 => self.right.lower_data[8],

                    49 => self.right.upper_data[0],
                    50 => self.right.upper_data[1],
                    51 => self.right.upper_data[2],
                    52 => self.right.upper_data[3],
                    53 => self.right.upper_data[4],
                    54 => self.right.upper_data[5],
                    ...
                    70 => self.left.addr[4],
                    71 => self.left.addr[5],
                    72 => if (addr_bits == 14) self.left.addr[6] else .no_connect,
                    76 => self.left.addr[if (addr_bits == 14) 7 else 6],
                    77 => self.left.addr[if (addr_bits == 14) 8 else 7],
                    78 => self.left.addr[if (addr_bits == 14) 9 else 8],
                    79 => self.left.addr[if (addr_bits == 14) 10 else 9],
                    80 => self.left.addr[if (addr_bits == 14) 11 else 10],
                    81 => self.left.addr[if (addr_bits == 14) 12 else 11],
                    82 => switch (addr_bits) {
                        12 => .no_connect,
                        13 => self.left.addr[12],
                        14 => self.left.addr[13],
                        else => unreachable,
                    },

                    59 => self.right.addr[0],
                    58 => self.right.addr[1],
                    57 => self.right.addr[2],
                    56 => self.right.addr[3],
                    55 => self.right.addr[4],
                    54 => if (addr_bits == 14) self.right.addr[5] else .no_connect,
                    50 => self.right.addr[if (addr_bits == 14) 6 else 5],
                    49 => self.right.addr[if (addr_bits == 14) 7 else 6],
                    48 => self.right.addr[if (addr_bits == 14) 8 else 7],
                    47 => self.right.addr[if (addr_bits == 14) 9 else 8],
                    46 => self.right.addr[if (addr_bits == 14) 10 else 9],
                    45 => self.right.addr[if (addr_bits == 14) 11 else 10],
                    44 => self.right.addr[if (addr_bits == 14) 12 else 11],
                    43 => switch (addr_bits) {
                        12 => .no_connect,
                        13 => self.right.addr[12],
                        14 => self.right.addr[13],
                        else => unreachable,
                    },

                    90 => self.left.lower_data[0],
                    91 => self.left.lower_data[1],
                    93 => self.left.lower_data[2],
                    94 => self.left.lower_data[3],
                    95 => self.left.lower_data[4],
                    96 => self.left.lower_data[5],
                    97 => self.left.lower_data[6],
                    98 => self.left.lower_data[7],
                    3 => if (byte_bits == 9) self.left.lower_data[8] else .no_connect,

                    99 => self.left.upper_data[0],
                    100 => self.left.upper_data[1],
                    5 => self.left.upper_data[2],
                    6 => self.left.upper_data[3],
                    7 => self.left.upper_data[4],
                    8 => self.left.upper_data[5],
                    10 => self.left.upper_data[6],
                    11 => self.left.upper_data[7],
                    4 => if (byte_bits == 9) self.left.upper_data[8] else .no_connect,

                    14 => self.right.lower_data[0],
                    15 => self.right.lower_data[1],
                    16 => self.right.lower_data[2],
                    18 => self.right.lower_data[3],
                    19 => self.right.lower_data[4],
                    20 => self.right.lower_data[5],
                    21 => self.right.lower_data[6],
                    26 => self.right.lower_data[7],
                    22 => if (byte_bits == 9) self.right.lower_data[8] else .no_connect,

                    27 => self.right.upper_data[0],
                    28 => self.right.upper_data[1],
                    29 => self.right.upper_data[2],
                    30 => self.right.upper_data[3],
                    31 => self.right.upper_data[4],
                    32 => self.right.upper_data[5],
                    33 => self.right.upper_data[6],
                    35 => self.right.upper_data[7],
                    23 => if (byte_bits == 9) self.right.upper_data[8] else .no_connect,
                ...
                packages.PLCC_84 => switch (@intFromEnum(pin_id)) {
                    ...
                    75 => self.left.addr[8],
                    76 => self.left.addr[9],
                    77 => self.left.addr[10],
                    78 => self.left.addr[11],
                    79 => if (addr_bits > 12) self.left.addr[12] else .no_connect,

                    60 => self.right.addr[0],
                    59 => self.right.addr[1],
                    58 => self.right.addr[2],
                    57 => self.right.addr[3],
                    56 => self.right.addr[4],
                    55 => self.right.addr[5],
                    54 => self.right.addr[6],
                    53 => self.right.addr[7],
                    52 => self.right.addr[8],
                    51 => self.right.addr[9],
                    50 => self.right.addr[10],
                    49 => self.right.addr[11],
                    48 => if (addr_bits > 12) self.right.addr[12] else .no_connect,

                    3 => self.left.lower_data[0],
                    4 => self.left.lower_data[1],
                    6 => self.left.lower_data[2],
                    7 => self.left.lower_data[3],
                    8 => self.left.lower_data[4],
                    9 => self.left.lower_data[5],
                    10 => self.left.lower_data[6],
                    11 => self.left.lower_data[7],

                    12 => self.left.upper_data[0],
                    13 => self.left.upper_data[1],
                    14 => self.left.upper_data[2],
                    15 => self.left.upper_data[3],
                    16 => self.left.upper_data[4],
                    17 => self.left.upper_data[5],
                    19 => self.left.upper_data[6],
                    20 => self.left.upper_data[7],

                    23 => self.right.lower_data[0],
                    24 => self.right.lower_data[1],
                    25 => self.right.lower_data[2],
                    27 => self.right.lower_data[3],
                    28 => self.right.lower_data[4],
                    29 => self.right.lower_data[5],
                    30 => self.right.lower_data[6],
                    31 => self.right.lower_data[7],

                    32 => self.right.upper_data[0],
                    33 => self.right.upper_data[1],
                    34 => self.right.upper_data[2],
                    35 => self.right.upper_data[3],
                    36 => self.right.upper_data[4],
                    37 => self.right.upper_data[5],
                    38 => self.right.upper_data[6],
                    40 => self.right.upper_data[7],
