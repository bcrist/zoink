...
pub fn finish_configuration(self: *Board, temp: std.mem.Allocator) !void {
    ...
    { // call check_config functions on any parts generated by check_config or get_or_generate_decouplers
        var i: usize = self.parts.items.len;
        while (i > num_regular_parts) {
            i -= 1;
            const p = self.parts.items[i];
            if (p.vt.check_config) |func| {
                try func(p.base, self);
    ...
    { // assign designators, default footprints, and BOM names
        ...
        for (self.parts.items) |p| {
            ...
            if (p.base.bom_name.len == 0) {
                p.base.bom_name = self.fmt("{t}: {s}", .{ p.base.prefix, p.base.value });
            }
...
pub fn generate_bom(self: *Board, temp: std.mem.Allocator, name: []const u8, writer: *std.Io.Writer) !void {
    const BOM_Part = struct {
        ...
        const BOM_Part_Self = @This();

        const Hash_Context = struct {
            pub fn hash(_: @This(), key: BOM_Part_Self) u32 {
                var h = std.hash.Wyhash.init(0);
                h.update(std.mem.asBytes(&key.name.len));
                h.update(key.name);
                h.update(std.mem.asBytes(&key.footprint_name.len));
                h.update(key.footprint_name);
                return @truncate(h.final());
            }
            pub fn eql(_: @This(), a: BOM_Part_Self, b: BOM_Part_Self, idx: usize) bool {
                _ = idx;
                return std.mem.eql(u8, a.name, b.name) and std.mem.eql(u8, a.footprint_name, b.footprint_name);
            }
        };
        const Sort_Context = struct {
            keys: []const BOM_Part_Self,

            pub fn lessThan(ctx: @This(), a_index: usize, b_index: usize) bool {
                const name_order = std.mem.order(u8, ctx.keys[a_index].name, ctx.keys[b_index].name);
                if (name_order != .eq) return name_order == .lt;
                return std.mem.lessThan(u8, ctx.keys[a_index].footprint_name, ctx.keys[b_index].footprint_name);
            }
        };
    };

    const Part_Hash_Context = struct {
        pub fn hash(key: *const Part.Base) u32 {
            var h = std.hash.Wyhash.init(0);
            h.update(std.mem.asBytes(&key.bom_name.len));
            h.update(key.bom_name);
            if (key.footprint) |fp| {
                h.update(std.mem.asBytes(&fp.name.len));
                h.update(fp.name);
            } else {
                const zero: usize = 0;
        ...
        pub fn eql(a: *const Part.Base, b: BOM_Part, idx: usize) bool {
            _ = idx;
            if (!std.mem.eql(u8, a.bom_name, b.name)) return false;

            var footprint_name: []const u8 = "";
            if (a.footprint) |fp| footprint_name = fp.name;

            return std.mem.eql(u8, footprint_name, b.footprint_name);
        }
    };

    var parts: std.ArrayHashMapUnmanaged(BOM_Part, usize, BOM_Part.Hash_Context, true) = .empty;
    defer parts.deinit(temp);

    for (self.parts.items) |p| {
        if (!p.base.include_in_bom or !p.base.populate) continue;

        const result = try parts.getOrPutAdapted(temp, p.base, Part_Hash_Context);
        if (result.found_existing) {
            result.value_ptr.* += 1;
        } else {
            result.key_ptr.* = .{
                .footprint_name = if (p.base.footprint) |fp| fp.name else "",
                .name = p.base.bom_name,
            };
            result.value_ptr.* = 1;
        }
    }
    ...
    try w.expression("bom");
    try w.string(name);
    w.set_compact(false);

    for (parts.keys(), parts.values()) |p, count| {
        try w.expression(p.name);
        if (p.footprint_name.len > 0) {
            try w.expression("fp");
            try w.string(p.footprint_name);
            try w.close();
        }
        try w.expression("qty");
        try w.int(count, 10);
        try w.close();
        try w.close();
    }
